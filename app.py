...
# cache last processed session so the dashboard-style
# pages can reuse stats/logs/rois without re-processing
last_roi_files = []
last_logs = ""
last_stats = _default_stats()
...
def index():
    global last_roi_files, last_logs, last_stats, last_video_filename
    ...
    if request.method == "POST":
        ...
        # update cache with most recent run
        last_roi_files = roi_files
        last_logs = logs
        last_stats = stats
        last_video_filename = video_filename
    else:
        # GET: reuse last processed results (or defaults)
        roi_files = last_roi_files
        logs = last_logs
        stats = last_stats
        video_filename = last_video_filename
